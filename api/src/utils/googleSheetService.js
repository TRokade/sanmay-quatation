...
const CONTACTSPREADSHEET_ID = "1OM5IIUhqEnRnWcAMzYGv2r-I-DZluoIAuBYew6AMkuQ";
const CONTACTSHEET_NAME = "Warsto Contact";

const auth = new google.auth.GoogleAuth({
  keyFile: path.join(
    __dirname,
    "../utils/quixotic-card-428217-m6-991872c2add4.json"
  ),
  scopes: SCOPES,
});
...
async function appendToSheet(formData, pdfBuffer) {
  try {
    ...
    const formattedOptions = Object.entries(formData.selectedOptions || {})
      .map(
        ([room, options]) =>
          `${room}: ${Array.isArray(options) ? options.join(", ") : options}`
      )
      .join("\n");
...
async function appendToContactSheet(formData) {
  try {
    ...
    const request = {
      spreadsheetId: CONTACTSPREADSHEET_ID,
      range: "A:E",
      valueInputOption: "USER_ENTERED",
      insertDataOption: "INSERT_ROWS",
      resource: { values },
